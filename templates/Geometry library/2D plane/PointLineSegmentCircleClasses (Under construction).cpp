...
public:
	...
	Polygon(const vector<Point> &other) :
			pts(other) {
	}

	Polygon(const initializer_list<Point> &other) :
			pts(other) {
	}

	Polygon& operator=(const vector<Point> &other) {
		this->pts = other;
		return *this;
	}

	bool operator==(const vector<Point> &other) const {
		return (this->pts == other);
	}
	...
	long double area() const { // O(pts.size())
		long double res = 0.0;
		for (int i = 0, n = pts.size(); i < n; ++i) {
			res += crossProduct(pts[i], pts[(i + 1 < n) ? i + 1 : 0]);
		}
		return abs(res) / 2.0;
	}

	bool isConvex() const { // O(pts.size())
		bool hasPos = false, hasNeg = false;
		for (int i = 0, n = pts.size(); i < n; ++i) {
			int o = orient(pts[i], pts[(i + 1) % n], pts[(i + 2) % n]);
			if (o > 0) hasPos = true;
			if (o < 0) hasNeg = true;
		}
	...
	void reverse(const unsigned int st, const unsigned int ed) {
		std::reverse(pts.begin() + st, pts.begin() + ed);
	}
	void reserve(const unsigned long long sz) {
		pts.reserve(sz);
	}
...
//i	f there are 0 tangents, it does nothing;
